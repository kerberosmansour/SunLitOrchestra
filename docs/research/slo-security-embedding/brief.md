...
Update `/slo-architect` to produce `SECURITY.md` + `docs/design/threat-model.md` as first-class artifacts alongside `ARCHITECTURE.md`, so every downstream SLO skill (`/slo-plan`, `/slo-critique`, `/slo-execute`, `/slo-verify`) has an upstream threat model to cite, generated — not prompted — per Google PSC's "remediate bug classes" framing and Jim Manico's `security.md` discipline.
