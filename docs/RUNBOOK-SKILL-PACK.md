...
> **Prerequisite reading**: [docs/skill-pack-catalog.md](skill-pack-catalog.md), [docs/runbook-template_v_3_template.md](runbook-template_v_3_template.md), [README.md](../README.md), [crates/sldo-common/src/preflight.rs](../crates/sldo-common/src/preflight.rs).
...
<!-- Completion summaries go in docs/completion/slo-sp-m<N>.md -->
...
│        ▼                            ▼                         ▼              │
│   docs/idea/, docs/research/,   docs/RUNBOOK-*.md,       Rust backends:      │
│   docs/design/,  specs/*.tla,   docs/lessons/,           sldo-research       │
│   docs/critique/, docs/verify/  docs/completion/         (primary, kept),    │
│                                                          sldo-plan, sldo-run │
│                                                          (batch fallback)    │
│                                                                              │
...
| `/slo-ideate` | YC-style product interrogation | M2 | Reads user pitch → writes `docs/idea/<slug>.md` |
| `/slo-retro` | Lessons + completion writer, tracker updater | M2 | Reads evidence log → writes `docs/lessons/*`, `docs/completion/*` |
| `/slo-research` | Wraps `sldo-research` | M3 | Reads idea doc → writes `docs/research/<slug>/*` |
| `/slo-architect` | Stack + ARCHITECTURE.md + interfaces lock-in | M4 | Reads idea + research → writes `ARCHITECTURE.md`, `docs/design/*` |
| `/slo-plan` | Interactive v3 runbook authoring | M4 | Reads architecture → writes `docs/RUNBOOK-<feat>.md` |
| `/slo-tla` | Formal verification via TLC/Apalache | M5 | Reads design → writes `specs/*.tla`, `specs/*.cfg`, `docs/design/*-verified.md` |
...
| `/slo-critique` | 4 sub-persona plan review | M6 | Reads runbook → writes `docs/critique/<slug>.md`, inline edits |
| `/slo-execute` | Per-milestone implementer | M7 | Reads milestone → writes tests + production code + evidence log entries |
| `/slo-verify` | Runtime + browser QA per milestone | M7 | Reads BDD scenarios → writes `docs/verify/*`, regression tests |
...
|---|---|---|---|---|
| Idea capture | user pitch | `docs/idea/<slug>.md` | skill writes file | M2 |
...
These rules apply to every milestone without exception. (Summarized; full text in [docs/runbook-template_v_3_template.md](runbook-template_v_3_template.md#global-execution-rules).)
...
1. Read the lessons file from the previous milestone (`docs/lessons/slo-sp-m<N-1>.md`).
...
11. Write `docs/lessons/slo-sp-m<N>.md`.
12. Write `docs/completion/slo-sp-m<N>.md`.
...
The v3 runbook template at [docs/runbook-template_v_3_template.md](runbook-template_v_3_template.md) is the crown jewel: BDD contracts, evidence logs, TLA+ section, lessons files, self-review gates. 11 runbooks have been authored against earlier template versions.
...
- The v3 runbook template ([docs/runbook-template_v_3_template.md](runbook-template_v_3_template.md)).
...
| Outputs | `docs/idea/<slug>.md`; `docs/lessons/<prefix>-m<N>.md`; `docs/completion/<prefix>-m<N>.md`; updated tracker |
...
| Files to read before changing anything | `docs/skill-pack-catalog.md`, `docs/runbook-template_v_3_template.md`, existing lessons files (`docs/lessons/research-m*.md`) as style reference |
...
| Scenario | Category | Given | When | Then |
|---|---|---|---|---|
| raw_pitch_produces_idea_doc | happy path | User provides 1-sentence pitch | `/slo-ideate` runs | `docs/idea/<slug>.md` exists with reframed pain, 3 approaches, recommended wedge |
...
| existing_idea_detected | empty state | `docs/idea/<slug>.md` already exists for this slug | `/slo-ideate` runs | Skill surfaces the overlap and asks whether to extend or start fresh |
...
**Goal**: Ship `/slo-research` as a thin skill that reads an idea doc, builds a research prompt, shells out to the `sldo-research` binary, and writes the structured dossier under `docs/research/<slug>/`. No reimplementation of research logic in the skill.
...
| Field | Value |
|---|---|
| Inputs | `docs/idea/<slug>.md` from M2 |
| Outputs | `docs/research/<slug>/dossier.md`, `sources.md`, `synthesis.md` |
...
| New dependencies allowed | `none` |
| Migration allowed | `no` (but if `sldo-research` output format changes, carry backward compatibility for existing `docs/research/*` files) |
...
| Scenario | Category | Given | When | Then |
|---|---|---|---|---|
| idea_doc_produces_dossier | happy path | Valid idea doc at `docs/idea/foo.md` | `/slo-research` runs | All three output files exist with required sections |
...
| Outputs | `ARCHITECTURE.md` (or updated), `docs/design/stack-decision.md`, `docs/design/interfaces.md`, `docs/RUNBOOK-<feature>.md` |
| Interfaces touched | New skills only; no Rust changes |
| Files allowed to change | `skills/slo-architect/**`, `skills/slo-plan/**` |
| Files to read before changing anything | `docs/runbook-template_v_3_template.md`, existing runbooks for style |
...
**Context**: No competitor skill pack does formal verification. This is SLO's differentiator. The v3 template already has a TLA+ section (lines 99–157 of [docs/runbook-template_v_3_template.md](runbook-template_v_3_template.md)) — this skill fills that section with a model-checked spec, not prose.
...
| Outputs | `specs/<name>.tla`, `specs/<name>.cfg`, `specs/<name>.trace.md` (if counterexamples), `docs/design/<name>-verified.md`; patches to runbook's TLA+ section if present |
...
| Files to read before changing anything | `crates/sldo-common/src/preflight.rs` (fail-loud pattern), `docs/runbook-template_v_3_template.md` (TLA+ section shape) |
...
| Outputs | Inline runbook edits; `docs/critique/<slug>.md` summary |
...
- Summary doc at `docs/critique/<slug>.md` cross-references each finding to a runbook line
...
| Files to read before changing anything | `docs/runbook-template_v_3_template.md`, gstack's `/qa` and `/ship` |
...
| Outputs | A new `docs/RUNBOOK-<feature>.md` produced entirely through the skill pipeline; `docs/critique/<feature>.md` from `/slo-critique` |
...
| 5 | Add `/slo-tla` + `~/.sldo/tla/` cache + external tool boxes | Add `/slo-tla` + TLA+ prereq section | Add `specs/*.out`, `states/` | `docs/design/tla-tool-cache.md` (new) |
