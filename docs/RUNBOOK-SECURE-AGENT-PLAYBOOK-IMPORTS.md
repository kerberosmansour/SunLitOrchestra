...
| Forbidden shortcuts | No grep-only review; the structural-contract test is mandatory. No copy-pasted inline finding template inside any SKILL.md. No "TODO: cite the template here" left in prose. No silent change to `references/security/security-{finding,assessment-summary}-template.md` table-row shapes. |
...
| Invariants/assertions required | (a) Every Markdown example has frontmatter with `synthetic: true`, `non-normative: true`, and `abbreviates: <path-or-name>`; (b) `examples/sast-manifest.json` declares `"synthetic": true` at top level; (c) PII regex scan over `examples/**/*.{md,json}` finds zero matches for email, UK NI, UK sort code; (d) every `abbreviates:` reference resolves to an existing path or known skill name; (e) `examples/README.md` lists all 6 artifact files. |
...
| `examples_pii_pattern_scan_clean` | No PII leakage | Regex scan over every `examples/**/*.{md,json}` for email, UK NI, UK sort code; zero matches |
| `every_example_under_size_cap` | Size bound holds | `wc -c` on every file ≤ 10 KB |
| `every_abbreviates_ref_resolves` | Citation invariant holds | Each `abbreviates:` ref must be either an existing path or a known skill name (`slo-*`) |
...
| Invariants/assertions required | (a) Every `uses:` reference in every `.github/workflows/*.yml` matches `^[a-zA-Z0-9._/-]+@[a-f0-9]{40}$` (SHA-pin); (b) if `plugin.json` exists, it does NOT duplicate skill files into a separate path — its skill paths point at the existing `skills/` tree; (c) `host-capability-matrix.md` contains a clear "green-lit / not green-lit / deferred" decision row plus reasoning; (d) the matrix names what each host supports for plugin install, agent install, and runtime invocation, with retrieval-date stamps; (e) if green-lit, the README "Examples" / "Install" sections mention the plugin path explicitly as optional and `sldo-install` as canonical. |
| Debugger / inspection expectation | Test failures cite the offending workflow file + line + the unpinned reference. Plugin.json structural failure cites the offending duplicated path. Decision-doc parse failure cites missing required section. |
...
| `.claude-plugin/plugin.json` | NEW (conditional): minimal manifest pointing at `skills/` tree; no duplication of source |
| `.github/workflows/release-zip.yml` | NEW (conditional): SHA-pinned `actions/checkout` + `actions/upload-artifact` (or equivalent); trigger on tag push only (`on: push: tags: ['v*']`); generates a release zip from the existing `skills/` + `references/` tree without modifying source |
| `xtasks/sast-verify/tests/sap_imp_m4_workflow_pinning.rs` | NEW: walks `.github/workflows/*.yml`, asserts every `uses:` matches `^[a-zA-Z0-9._/-]+@[a-f0-9]{40}$`. Asserts plugin.json (if exists) contains no `"path":` entries pointing into `.claude-plugin/skills/` (only `skills/` from repo root). Asserts release workflow trigger is tag-based, not push-based. |
...
| E2E Test | What It Proves | Pass Criteria |
|---|---|---|
| `every_workflow_uses_is_sha_pinned` | SHA-pin invariant holds across all workflows | Walks `.github/workflows/*.yml`; every `uses:` matches `^[a-zA-Z0-9._/-]+@[a-f0-9]{40}$` |
...
| `release_workflow_is_tag_triggered` | Trigger discipline holds | If `.github/workflows/release-zip.yml` exists, its `on:` block contains a tag-based trigger and no push-to-default-branch trigger |
...
| Invariants/assertions required | (a) Exactly 4 agent files in `agents/` if directory exists; (b) every agent file has frontmatter with `name`, `role`, `output-paths`, `copilot-fallback`, `host-required`; (c) every `output-paths` entry is in `{docs/slo/critique/, docs/slo/verify/}`; (d) every agent has a non-empty `copilot-fallback` field naming either "supported per matrix" or a specific feature-flag fallback; (e) every agent file ≤ 200 lines; (f) `/slo-critique` SKILL.md unchanged (the canonical portable path is preserved); (g) (Deferred) the lessons file documents the matrix gate that fired and includes a fresh-runbook handoff note. |
...
| `every_output_path_in_allowed_set` | Output-path invariant holds | Each `output-paths:` entry ∈ `{docs/slo/critique/, docs/slo/verify/}` |
...
| `slo_critique_skill_md_unchanged` | Canonical portable path preserved | `skills/slo-critique/SKILL.md` byte-identical to its M4 baseline (asserted via a hash recorded in the test or a diff against `git show HEAD:skills/slo-critique/SKILL.md`) — alternative: assert the file's frontmatter `name` and the canonical persona-rotation prose section are present |
