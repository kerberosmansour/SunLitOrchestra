...
|---|---|---|---|---|---|---|
| 1 | `/slo-sast` SKILL.md scaffold + threat-model parser (CWE extraction with comment / fence scope rule) | `done` | 2026-04-26 | 2026-04-26 | [docs/lessons/scanner-orch-m1.md](lessons/scanner-orch-m1.md) | [docs/completion/scanner-orch-m1.md](completion/scanner-orch-m1.md) |
| 2 | Stack detection + `semgrep-rules` cache fetch at pinned SHA + CWE × technology rule filter | `done` | 2026-04-26 | 2026-04-26 | [docs/lessons/scanner-orch-m2.md](lessons/scanner-orch-m2.md) | [docs/completion/scanner-orch-m2.md](completion/scanner-orch-m2.md) |
| 3 | Emit `.semgrep/rules/`, `.semgrep.yml`, `.github/workflows/sast.yml` with workflow-safety structural-contract test | `done` | 2026-04-26 | 2026-04-26 | [docs/lessons/scanner-orch-m3.md](lessons/scanner-orch-m3.md) | [docs/completion/scanner-orch-m3.md](completion/scanner-orch-m3.md) |
| 4 | Emit `.semgrep/manifest.json` (audit-defense schema v1.0) + initial-baseline preview-mode UX | `done` | 2026-04-26 | 2026-04-26 | [docs/lessons/scanner-orch-m4.md](lessons/scanner-orch-m4.md) | [docs/completion/scanner-orch-m4.md](completion/scanner-orch-m4.md) |
| 5 | Re-derivation trigger detection + diff PR generation + dogfood E2E against this SLO repo | `done` | 2026-04-26 | 2026-04-26 | [docs/lessons/scanner-orch-m5.md](lessons/scanner-orch-m5.md) | [docs/completion/scanner-orch-m5.md](completion/scanner-orch-m5.md) |
...
| `skills/slo-sast/SKILL.md` written | manual review | frontmatter valid, parse contract cited | `name: slo-sast`; description (multi-line `>`) summarizes role; cites `references/sast/threat-model-parser-contract.md`, `docs/design/scanner-orchestration-threat-model.md`, `docs/design/scanner-orchestration-interfaces.md`, `docs/RUNBOOK-SCANNER-ORCHESTRATION.md`; M1 anti-patterns section forbids M2+ behaviors | **PASS** | |
| Discovery check | `./target/release/sldo-install --local --dry-run` | `slo-sast` listed | `+ /...SunLitOrchestrate/.claude/skills/slo-sast -> .../skills/slo-sast` (31 skills total — slo-sast added) | **PASS** | |
...
| Test artifact cleanup | `git status` | only intended new files; no untracked test output | New files: `crates/sldo-install/tests/e2e_scanner_orch_m1.rs`, `references/sast/threat-model-parser-contract.md`, `skills/slo-sast/SKILL.md`; modified: `docs/RUNBOOK-SCANNER-ORCHESTRATION.md`. No untracked test output. | **PASS** | |
...
| Files to read before changing anything | M1 + M2 outputs (`skills/slo-sast/SKILL.md`, both reference docs, both M1+M2 test files); `docs/lessons/scanner-orch-m1.md`, `docs/lessons/scanner-orch-m2.md`; `docs/design/scanner-orchestration-interfaces.md` (§§4, 6); `docs/design/scanner-orchestration-threat-model.md` (abuse case `tm-scanner-orchestration-abuse-3` + the AI/LLM section); [SECURITY.md](../SECURITY.md) "Scanner orchestration skill — additional rules" → "Emitted-workflow safety contract" subsection (the asserted properties); [docs/research/scanner-orchestration/synthesis.md](research/scanner-orchestration/synthesis.md) (§ "fetch-depth: 0 is mandatory", "SHA-pin every action", "`pull_request_target` ban survives the 2025-12-08 mitigation") |
...
1. Complete the Global Entry Rules.
2. Read `docs/lessons/scanner-orch-m1.md` AND `docs/lessons/scanner-orch-m2.md`; apply relevant corrections.
3. Read the allowed files before editing.
4. Copy the Evidence Log template into this milestone section or working notes.
...
3. Invoke `claude /slo-sast /tmp/scanner-orch-smoke-m3/docs/design/threat-model.md` (or equivalent).
...
- [ ] Lessons file at `docs/lessons/scanner-orch-m3.md`
- [ ] Completion summary at `docs/completion/scanner-orch-m3.md`
- [ ] Milestone Tracker row updated to `done`
...
| Files to read before changing anything | M1 + M2 + M3 outputs; `docs/lessons/scanner-orch-m1.md`, `m2.md`, `m3.md`; `docs/design/scanner-orchestration-interfaces.md` (§5); `docs/design/scanner-orchestration-threat-model.md` (abuse cases `tm-scanner-orchestration-abuse-4`, `tm-scanner-orchestration-abuse-5`); [SECURITY.md](../SECURITY.md) "Scanner orchestration skill — additional rules" → "Manifest schema is audit-defense, not regulatory necessity" subsection; `docs/research/scanner-orchestration/synthesis.md` (§ "Audit defense via per-scan-run manifest is defensive, not regulatory") |
...
- [ ] Lessons file at `docs/lessons/scanner-orch-m4.md`
- [ ] Completion summary at `docs/completion/scanner-orch-m4.md`
- [ ] Milestone Tracker row updated to `done`
...
| Files to read before changing anything | M1 + M2 + M3 + M4 outputs; all four prior lessons files; [interfaces.md §8](design/scanner-orchestration-interfaces.md); `docs/research/scanner-orchestration/synthesis.md` (§ "wedge is genuinely unoccupied — frame as first-of-its-kind"); existing skills using `gh` CLI (search for `gh issue create` / `gh pr create` patterns elsewhere in `skills/`); the `/slo-sec-libs` precedent for rate-limit discipline (referenced in issue #4 body) |
...
| `dogfood_against_slo_repo` | happy path (dogfood) | the dogfood-subtree fixture mirrors this SLO repo's `docs/design/scanner-orchestration-*.md` | the skill is invoked against the dogfood subtree (NOT the real repo) | the full M1→M5 pipeline runs end-to-end; emitted artifacts are valid; manifest is populated; no real PR is opened against `kerberosmansour/SunLitOrchestrate` (the test-stubbed `gh` only records — the test asserts no real network request occurred) |
...
2. Run `./target/release/sldo-install --local`.
3. Invoke `claude /slo-sast` against THIS repo using `docs/design/scanner-orchestration-threat-model.md`.
...
- [ ] Lessons file at `docs/lessons/scanner-orch-m5.md`
- [ ] Completion summary at `docs/completion/scanner-orch-m5.md`
