...
| `docs/slo/templates/runbook-template_v_3_template.md` | Adds "Carry-forward from prior retros" section | M4 | Read by `/slo-plan` and `/slo-resume` |
...
/slo-retro M<N>
  ┌─────────────────────────────────┐
  │ Write docs/slo/lessons/<prefix>-m<N>.md │
  └──────────────┬──────────────────┘
                 │
                 ▼
        ┌────────────────┐
...
- Issue body content originates from `docs/slo/lessons/<prefix>-m<N>.md` — author-controlled, not user-input. Low injection risk; argv-list discipline is the standard defense.
