...
1. **In M4's release workflow body**: use `git archive --format=zip --prefix=sunlit-orchestrate-${TAG}/ HEAD -o release.zip`. This emits *only* tracked files at the tagged SHA; untracked and gitignored content is excluded by construction.
