...
- `docs/slo/design/agent-host-capabilities.md` added a per-skill notes section covering `/slo-second-opinion`, `/slo-research`, `/slo-rulegen`, `/slo-sast`, and the live business judgment runtime. Capability matrix renamed the batch-backend row to "Optional Claude batch backend" and noted the helper module path explicitly for the live runtime row.
