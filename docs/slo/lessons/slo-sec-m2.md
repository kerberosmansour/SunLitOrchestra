...
- `crates/sldo-install/tests/e2e_slo_sec_m2.rs` (NEW) — 16 structural-contract tests, including a non-cryptographic FNV-1a-64 invariant that catches any byte edit to `docs/slo/templates/runbook-template_v_3_template.md`.
...
- **Read `docs/slo/lessons/slo-sec-m1.md` and `slo-sec-m2.md` before writing M3 tests.** M1 established the reference-templates-under-`references/` convention; M2 established the FNV-1a invariant pattern. M3 uses both.
...
- `docs/slo/templates/runbook-template_v_3_template.md` could have a sample Contract Block that already includes the three new rows — but that would require a runbook-template version bump, which the contract forbids in this runbook. Deferred.
