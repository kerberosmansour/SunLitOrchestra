...
- **NEW** `docs/slo/verify/biz-a-m2-smoke.md` — manual smoke checklist with 6 fixtures (R&D claim, VAT triage, HMRC filing hard-block, GDPR cross-skill routing, cross-skill predicate consistency, non-UK jurisdiction).
- **UPDATED** `docs/ARCHITECTURE.md` — added `slo-accounting` row to the skill table.
- **UPDATED** `docs/slo/completed/RUNBOOK-BIZ-SKILL-PACK-A.md` Milestone Tracker — M2 status `done`.
