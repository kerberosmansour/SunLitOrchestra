...
> **Prerequisite reading**: [docs/ARCHITECTURE.md](ARCHITECTURE.md), [docs/idea/slo-security-embedding.md](idea/slo-security-embedding.md), [docs/research/slo-security-embedding/synthesis.md](research/slo-security-embedding/synthesis.md), [docs/design/slo-security-embedding-overview.md](design/slo-security-embedding-overview.md), [docs/design/slo-security-embedding-stack-decision.md](design/slo-security-embedding-stack-decision.md), [docs/design/slo-security-embedding-interfaces.md](design/slo-security-embedding-interfaces.md)
...
- **Public interfaces that must remain stable unless explicitly listed otherwise**:
  ...
  - `docs/design/<slug>-overview.md` frontmatter key `tla_required` (read by `/slo-tla`)
...
|---|---|---|---|---|---|---|
| 1 | `/slo-ideate` risk question + `/slo-architect` emits `SECURITY.md` + threat-model.md | `done` | 2026-04-24 | 2026-04-24 | [docs/lessons/slo-sec-m1.md](lessons/slo-sec-m1.md) | [docs/completion/slo-sec-m1.md](completion/slo-sec-m1.md) |
| 2 | `/slo-plan` Contract Block expansion (data classification, proactive controls, abuse cases) | `done` | 2026-04-24 | 2026-04-24 | [docs/lessons/slo-sec-m2.md](lessons/slo-sec-m2.md) | [docs/completion/slo-sec-m2.md](completion/slo-sec-m2.md) |
| 3 | `/slo-critique` security persona rewrite (class elimination + variant analysis) | `done` | 2026-04-24 | 2026-04-24 | [docs/lessons/slo-sec-m3.md](lessons/slo-sec-m3.md) | [docs/completion/slo-sec-m3.md](completion/slo-sec-m3.md) |
| 4 | `/slo-verify` Pass 4 (supply-chain + variant-analysis spot check) | `done` | 2026-04-24 | 2026-04-24 | [docs/lessons/slo-sec-m4.md](lessons/slo-sec-m4.md) | [docs/completion/slo-sec-m4.md](completion/slo-sec-m4.md) |

<!-- Status values: not_started | in_progress | blocked | done -->
<!-- Lessons files go in docs/lessons/slo-sec-m<N>.md -->
<!-- Completion summaries go in docs/completion/slo-sec-m<N>.md -->

---

## End-to-End Architecture Diagram

See [docs/design/slo-security-embedding-overview.md](design/slo-security-embedding-overview.md) for the full diagram with legend. Summary view below; solid = exists today, dashed = added by this runbook.
...
│                                                                             │
│  slo-ideate       ───► docs/idea/<slug>.md                                  │
│                    ─ ► + Top risks block (M1)                               │
│                                                                             │
│  slo-research     ───► docs/research/<slug>/{dossier,sources,synthesis}.md  │
│                                                                             │
│  slo-architect    ───► docs/ARCHITECTURE.md (reality)                       │
│                    ───► docs/design/<slug>-{overview,stack,interfaces}.md   │
│                    ─ ► SECURITY.md (M1, repo root, generated)               │
│                    ─ ► docs/design/<slug>-threat-model.md (M1)              │
...
|---|---|---|---|---|
| Idea-doc risk framing | `/slo-ideate` | `docs/idea/<slug>.md` | File write (Markdown) | M1 |
| Project security rules | `/slo-architect` | `SECURITY.md` (repo root) | File write (Markdown, generated from template) | M1 |
| Threat model | `/slo-architect` | `docs/design/<slug>-threat-model.md` | File write (Markdown, generated from template + STRIDE sweep) | M1 |
| Security frontmatter | `/slo-architect` | `docs/design/<slug>-overview.md` | File write (YAML frontmatter keys) | M1 |
| Contract security rows | `/slo-plan` | `docs/RUNBOOK-<slug>.md` | File write (Markdown tables) | M2 |
| Bug-class findings | `/slo-critique` | `docs/critique/<slug>.md` | File write (Markdown table; cites threat-model rows) | M3 |
| Security verification rows | `/slo-verify` | `docs/verify/<slug>-m<N>.md` + milestone Evidence Log | File write (Markdown) + subprocess invocation (`cargo audit`, `cargo deny`, `semgrep`, `ast-grep`) | M4 |
...
**N/A** — justification from [docs/design/slo-security-embedding-overview.md](design/slo-security-embedding-overview.md) frontmatter: "Skill-pack edits and markdown authoring. No concurrent actors sharing state, no distributed consensus, no leader election, no cross-process ordering guarantees, no resource leases. Every phase is sequential file I/O plus subprocess invocation."
...
- Do not modify `docs/runbook-template_v_3_template.md` — backward compatibility with existing runbooks (RUNBOOK-AWS-ORG-SETUP, RUNBOOK-API-FACADE, etc.) is load-bearing.
...
- Existing critique findings (`docs/critique/*.md`) must still match the reshaped security persona's output format.
...
1. Read the lessons file from the previous milestone (`docs/lessons/slo-sec-m<N-1>.md`), if one exists.
...
11. Write `docs/lessons/slo-sec-m<N>.md`.
12. Write `docs/completion/slo-sec-m<N>.md`.
...
See the End-to-End Architecture Diagram above and [docs/design/slo-security-embedding-overview.md](design/slo-security-embedding-overview.md).

### Key Design Principles

1. **Threat model is generated, not prompted.** `/slo-architect` emits `docs/design/<slug>-threat-model.md` in one pass; the user reviews. 80/20 burden (Google PSC).
...
- `docs/runbook-template_v_3_template.md` — do not modify; backward compat with existing runbooks.
...
Path: `docs/lessons/slo-sec-m<N>.md`. Use the shape from `docs/runbook-template_v_3_template.md`.

## Completion Summary Template

Path: `docs/completion/slo-sec-m<N>.md`. Use the shape from `docs/runbook-template_v_3_template.md`.
...
**Goal**: `/slo-ideate` captures top security risks during ideation; `/slo-architect` emits a project-wide `SECURITY.md` and a per-feature `docs/design/<slug>-threat-model.md` as first-class artifacts, and sets `security_libs_required`, `ai_component`, and `compliance` frontmatter on `<slug>-overview.md`.
...
| Field | Value |
|---|---|
| Inputs | `docs/idea/<slug>.md` (for ideate-triggered runs; must now include Top risks block after M1 ships); `docs/research/<slug>/synthesis.md`; target repo manifests for stack detection (Cargo.toml / package.json / go.mod / pyproject.toml / Gemfile) |
| Outputs | Updated `skills/slo-ideate/SKILL.md`; updated `skills/slo-architect/SKILL.md`; new `skills/slo-architect/references/SECURITY-md-template.md`; new `skills/slo-architect/references/threat-model-template.md`; new `crates/sldo-install/tests/e2e_slo_sec_m1.rs`. Runtime artifacts produced by the updated skills (post-ship): `SECURITY.md` at target repo root, `docs/design/<slug>-threat-model.md`, new frontmatter fields on `docs/design/<slug>-overview.md`. |
| Interfaces touched | `/slo-ideate` verb (unchanged); idea-doc shape (additive — new "Top risks" block); `/slo-architect` verb (unchanged); `<slug>-overview.md` frontmatter (three new keys: `security_libs_required`, `ai_component`, `compliance`); new artifact paths `SECURITY.md` and `docs/design/<slug>-threat-model.md`; tests harness (new `[[test]]` entry). |
...
| Files to read before changing anything | `skills/slo-ideate/SKILL.md` (baseline prose + idea-doc shape); `skills/slo-architect/SKILL.md` (baseline steps); `skills/slo-critique/personas/security.md` (to know what downstream consumes); `docs/design/slo-security-embedding-overview.md` (for the frontmatter contract); `docs/runbook-template_v_3_template.md` (to ensure no template drift); `docs/research/slo-security-embedding/synthesis.md` (for SOC 2 + ASVS default, AI triad, STRIDE stability requirement); `Cargo.toml` root (to see existing `[[test]]` entries and copy the convention). |
...
- No changes to `docs/runbook-template_v_3_template.md`.
...
| ideate SKILL.md backward compat | backward compat | an existing idea doc without a Top risks block (`docs/idea/tla-sha-autopop.md`) | parsing it with the v2-compatible idea-doc parser if one exists, else just `read_to_string` | the file still reads successfully and the updated SKILL.md does not imply deletion of existing sections |
| architect SKILL.md backward compat | backward compat | the pre-edit overview (`docs/design/tla-sha-autopop-overview.md` or any existing overview without `security_libs_required`) | reading it through the updated architect's `<slug>-overview.md` handling | the architect treats the absent `security_libs_required` as `false` (default) without erroring |
| absent idea doc on architect run | invalid input | a target with no `docs/idea/<slug>.md` | running the conceptual `/slo-architect` path described by the edited SKILL.md | the SKILL.md still documents behavior for that case (either refusal or fallback) and the prose is unchanged where applicable |
...
- Existing idea docs (`docs/idea/tla-sha-autopop.md`, `docs/idea/slo-security-embedding.md`) continue to read as valid idea-doc shape.
...
- [ ] `docs/runbook-template_v_3_template.md` not modified (byte-for-byte identical to pre-milestone).
...
| `existing_overview_frontmatter_valid` | backward-compat invariant | `docs/design/tla-sha-autopop-overview.md` frontmatter still parses (no `security_libs_required` required) |
...
| Smoke tests | `/slo-ideate` manual + `/slo-architect` dogfood + `sldo-install --dry-run` | all checked | Dogfood smoke: `SECURITY.md` generated at SLO repo root (~180 lines, follows template); `docs/design/slo-security-embedding-threat-model.md` generated (~140 lines, STRIDE + abuse cases + AI triad + residual risks); `sldo-install --dry-run` reports all skills up to date | **pass** | `/slo-ideate` manual on a disposable tempdir skipped — the ideate SKILL.md edits are exercised by the structural-contract tests asserting the shape. |
...
- Lessons file `docs/lessons/slo-sec-m1.md` written.
- Completion summary `docs/completion/slo-sec-m1.md` written.
...
| Field | Value |
|---|---|
| Inputs | A runbook draft in progress by `/slo-plan`; a threat model at `docs/design/<slug>-threat-model.md` produced by M1's `/slo-architect`; a stack-decision doc for control vocabulary selection. |
...
| Files to read before changing anything | `skills/slo-plan/SKILL.md` (baseline); `docs/runbook-template_v_3_template.md` (must not be modified); `skills/slo-architect/references/threat-model-template.md` (produced in M1 — the source for abuse-case rows); `skills/slo-critique/personas/security.md` (knows how downstream cites threat-model rows); `docs/design/slo-security-embedding-interfaces.md` (the schema contract for the three new rows); `Cargo.toml` root for `[[test]]` convention. |
...
| Compatibility commitments | `docs/runbook-template_v_3_template.md` is not modified. Existing runbooks (`RUNBOOK-AWS-ORG-SETUP.md`, `RUNBOOK-API-FACADE.md`, `RUNBOOK-TLA-SHA-AUTOPOP.md`, `RUNBOOK-RESEARCH.md`, etc.) continue to parse through `sldo-common::runbook`. `/slo-plan` when invoked on an old slug continues to work — the three new rows are required only for newly-authored milestones. |
| Forbidden shortcuts | No editing of `docs/runbook-template_v_3_template.md`. No cross-skill edits (M3's critique persona rewrite is separate). No inlining of the vocabulary / examples into `SKILL.md` — they go in `references/` to stay maintainable. No accepting a milestone Contract Block with a silently-missing row; the skill must produce "N/A — <reason>" rows for no-new-surface cases. No invention of new OWASP proactive-control category names — use the canonical C1–C10 mapping from SunLitSecureLibraries for Rust-axum targets and OWASP Proactive Controls v3 names for other stacks. |
...
1. Complete Global Entry Rules.
2. Read `docs/lessons/slo-sec-m1.md` (from the just-closed M1) and apply any rules-for-next-milestone corrections.
...
5. Re-state the milestone constraints:
   ...
   - forbidden: any change to `docs/runbook-template_v_3_template.md` or any other skill.
...
- `docs/runbook-template_v_3_template.md` byte-identical pre/post-milestone.
...
- [ ] `docs/runbook-template_v_3_template.md` byte-identical.
...
| `template_not_modified` | template invariant | `docs/runbook-template_v_3_template.md` SHA-256 matches an **inline const `EXPECTED_RUNBOOK_TEMPLATE_SHA256` declared in the test source** (computed once when M2 begins via `sha256sum docs/runbook-template_v_3_template.md` and pasted into the test). Any unintended edit to the template flips the hash and fails this test. |
...
- `docs/lessons/slo-sec-m2.md` written.
- `docs/completion/slo-sec-m2.md` written.
- Milestone Tracker updated.
...
**Goal**: `skills/slo-critique/personas/security.md` is rewritten so every accepted finding (a) names a bug *class* (not an instance), (b) states whether the design eliminates the class or only mitigates it, (c) cites a row from `docs/design/<slug>-threat-model.md`, and (d) includes a one-line variant-analysis directive ("grep for same-pattern variants at `<locations>`").
...
| Field | Value |
|---|---|
| Inputs | A runbook at `docs/RUNBOOK-<slug>.md`; a threat model at `docs/design/<slug>-threat-model.md` (produced in M1); an architecture diagram for variant-analysis target selection. |
...
| Files to read before changing anything | `skills/slo-critique/personas/security.md` (baseline — understand what to preserve); `skills/slo-critique/SKILL.md` (handoff context); `skills/slo-critique/personas/{ceo,eng,design}.md` (to match persona-prompt style conventions); `docs/design/slo-security-embedding-threat-model.md` (produced in M1 — understand the citation target shape); `skills/slo-architect/references/threat-model-template.md` (the template the persona cites); `docs/research/slo-security-embedding/synthesis.md` (for the class-elimination framing from Google PSC). |
...
| New dependencies allowed | `none` |
| Migration allowed | `no` — finding-row table schema unchanged; existing `docs/critique/*.md` files continue to render. |
| Compatibility commitments | `/slo-critique` continues to run the same four personas in the same rotation order; only the `security.md` content changes. Existing `docs/critique/*.md` outputs remain readable. The security persona still emits findings into the same table schema. |
...
- No new runtime dependencies.
- No changes to `docs/runbook-template_v_3_template.md`.
- No retroactive rewrite of existing `docs/critique/*.md` outputs.

#### Pre-Flight

1. Complete Global Entry Rules.
2. Read `docs/lessons/slo-sec-m2.md` and apply corrections.
...
| persona requires threat-model citation | happy path | same | scanning for citation language | text requires every finding to cite a row from `docs/design/<slug>-threat-model.md` |
...
| existing critiques still render | backward compat | `docs/critique/*.md` files | reading them | files remain valid Markdown with the same finding-row table schema |
...
- Baseline suite green.
- Existing `docs/critique/*.md` files parseable as Markdown.
...
- [ ] Existing `docs/critique/*.md` outputs continue to render.
- [ ] `sldo-install --dry-run` clean.
...
| `persona_requires_threat_model_citation` | citation gate present | text references `docs/design/<slug>-threat-model.md` as a required input |
...
| `existing_critiques_valid_markdown` | backward compat | each `docs/critique/*.md` parses through `pulldown-cmark` (or regex proxy) without error |
...
| Compatibility checks | ceo/eng/design unchanged, finding-row unchanged, existing critique readable | no regressions | 4 invariant tests pass: ceo/eng/design FNV-1a hashes match; finding-row header identical; `docs/critique/tla-sha-autopop.md` still contains finding-row table | **pass** | |
...
- `docs/lessons/slo-sec-m3.md` written.
- `docs/completion/slo-sec-m3.md` written.
- Milestone Tracker updated.
...
| Files to read before changing anything | `skills/slo-verify/SKILL.md` (baseline three-pass structure); `docs/design/slo-security-embedding-overview.md` (Phase-scope boundary — M4 is Pass 4 only, not the `/slo-security-test` skill); `docs/research/slo-security-embedding/synthesis.md` (Semgrep + ast-grep + CodeQL pairing rationale); `Cargo.toml` root; `crates/secure_*` (any smoke-service references — there are none in the SLO repo itself, so DAST path is mostly exercised as the "no smoke service" N/A case here). |
...
| Migration allowed | `no`. |
| Compatibility commitments | `/slo-verify` continues to run passes 1–3 unchanged. Existing `docs/verify/*.md` reports remain valid. When Pass 4 runs against a target where no tools are installed, it emits an all-skipped Pass 4 section with reasons — does not fail the verification. |
...
1. Complete Global Entry Rules.
2. Read `docs/lessons/slo-sec-m3.md`.
3. Read allowed + required files.
4. Copy Evidence Log template.
5. Re-state constraints:
...
- Existing `docs/verify/*.md` files (none exist in this repo currently, but the test is future-proofing) parse.
...
- `docs/lessons/slo-sec-m4.md` written.
- `docs/completion/slo-sec-m4.md` written.
- Milestone Tracker updated.
