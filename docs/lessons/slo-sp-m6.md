...
- Critique output: `docs/critique/<runbook-slug>.md`, one row per finding, columns: `id | persona | category | runbook section | finding | concrete scenario | recommendation`.
...
- A test that `/slo-critique` writes a `docs/critique/*.md` file with the required column headers.
