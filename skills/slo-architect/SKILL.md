...
5. **Emit `docs/slo/design/<slug>-threat-model.md`** using `references/threat-model-template.md`. Same `~~~text` fence rule for user strings. Populate the AI-specific section only when `ai_component: true`, and name any AI tolerance assumptions downstream milestone contracts must prove. GDPR gets both a column and a section when `gdpr` is in the `compliance:` list.
...
8. **Also emit `docs/slo/design/<slug>-threat-model.slo.json`** — the machine-readable companion, conforming to [`references/security/threat-model-schema.md`](../../references/security/threat-model-schema.md) (frozen schema; `slo_schema_version: 0.1.0` — do not change the schema). Serialize the same STRIDE / abuse-case / residual / compliance content the Markdown threat model carries. **Provenance:** set `provenance.producer_skill_sha` to the git SHA of this `SKILL.md` at emit time and each `provenance.inputs[].sha` to the git blob SHA of the idea / research / architecture input — same idiom as `references/sast/manifest-schema.md` (a producing-skill SKILL.md git sha, NOT a content hash). **Frozen IDs:** every abuse case keeps its `tm-<slug>-abuse-N` id 1:1 with the Markdown row; on re-run **supersede-don't-renumber** (set `status: superseded` + non-empty `superseded_by` + `supersede_reason`; never renumber, never silently drop a superseded row). **SEC-1 producer-side neutralisation (write-side analogue of the consumer fence rule):** emit the JSON through a structural JSON serializer (or escape equivalently) so a crafted idea-doc string cannot break out of a JSON string; user-controlled idea-doc / Top-risks text is rendered ONLY into descriptive string fields and **never** chooses `id`, `classification`, `accepted_residual`, or `status` — those are author-controlled. Apply the same idempotency rule as item 7 to the `.slo.json` (detect / diff / overwrite-merge-skip; no silent clobber).
