...
| SAST→DAST route bridge | When SARIF lacks routes, **the agent reads the codebase** to resolve each sink to an entry point and emits an agent-resolved guided map. Read [`references/sast-route-bridge.md`](references/sast-route-bridge.md) (overview + Express, validated:NodeGoat: 0→4 confirmed), then the generalized machinery: [`references/entry-point-map-contract.md`](references/entry-point-map-contract.md) (protocol-abstract schema), [`references/resolver-adapter-contract.md`](references/resolver-adapter-contract.md) (adapter spec + **generic fallback** — unknown stacks degrade, never break), [`references/resolvers/index.md`](references/resolvers/index.md) (12-framework catalog), [`references/probe-library.md`](references/probe-library.md), and the shared [`vuln-class-taxonomy`](../../references/security/vuln-class-taxonomy.md). Pick the adapter from detected stack; `spec-only` adapters are hints (label `adapter:spec-only`), never invent an entry point. |
