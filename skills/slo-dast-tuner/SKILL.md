...
| SAST→DAST route bridge | When SARIF lacks routes, **the agent reads the codebase** to resolve each sink to method+url+auth and emits an agent-resolved guided map. Read [`references/sast-route-bridge.md`](references/sast-route-bridge.md). This is the load-bearing middle ground: validated on NodeGoat it turned 0 confirmed (blind baseline) into 4 confirmed (IDOR/redirect/SSRF/ReDoS) via targeted authenticated probes. |
