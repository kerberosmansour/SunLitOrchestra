...
Installer CLI for the [SunLit Orchestra](https://github.com/kerberosmansour/SunLitOrchestra) `/slo-*` skill pack — installs into Claude Code, Codex, or GitHub Copilot host roots.

## When to reach for this crate

You want to use the SunLit Orchestra `/slo-*` skill pack inside an interactive AI coding host (Claude Code, Codex, Copilot), and you need to **link the skills under `skills/` in the repo into the host's skills directory** without copying or hand-editing config.
...
- **Multi-host** — pick `claude-code` (default), `codex`, or `copilot` per invocation.
...
# Install into a project-local Codex skills directory (./.codex/skills/).
sldo-install --host codex --local install
...
| Flag | Purpose |
|---|---|
| `--host <claude-code\|codex\|copilot>` | Target host agent. Default: `claude-code`. |
...
- MSRV: 1.75
- macOS, Linux, Windows
- Hosts: Claude Code (`~/.claude/skills/`), Codex (`~/.codex/skills/`), GitHub Copilot (`~/.copilot/skills/`)
