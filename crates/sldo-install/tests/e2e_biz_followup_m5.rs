...
fn matches_email(s: &str) -> bool {
    for token in s.split_whitespace() {
        ...
        let Some(at_idx) = token.find('@') else { continue };
        let (local, after_at) = token.split_at(at_idx);
        let domain = &after_at[1..];
        if local.is_empty() { continue; }
        // Local part chars: alphanumeric + . _ - + %
        if !local.chars().all(|c| c.is_ascii_alphanumeric() || ".-_+%".contains(c)) { continue; }
        // Domain must contain a '.'
        let Some(dot_idx) = domain.find('.') else { continue };
        if dot_idx == 0 || dot_idx == domain.len() - 1 { continue; }
        // TLD ≥ 2 alpha chars.
        let tld_start = domain.rfind('.').unwrap() + 1;
        let tld = &domain[tld_start..];
        if tld.len() < 2 { continue; }
        if !tld.chars().all(|c| c.is_ascii_alphabetic()) { continue; }
        // Domain pre-TLD chars: alphanumeric + . -
        let domain_pre = &domain[..tld_start - 1];
        if !domain_pre.chars().all(|c| c.is_ascii_alphanumeric() || ".-".contains(c)) { continue; }
        return true;
    }
    false
}
...
fn matches_uk_ni(s: &str) -> bool {
    ...
    for i in 0..chars.len().saturating_sub(8) {
        ...
        if i > 0 && chars[i - 1].is_ascii_alphanumeric() { continue; }
        // 9 chars: 2 letters, 6 digits, 1 letter.
        if !valid_first.contains(chars[i]) { continue; }
        if !valid_second.contains(chars[i + 1]) { continue; }
        if !chars[i + 2..i + 8].iter().all(|c| c.is_ascii_digit()) { continue; }
        if !valid_suffix.contains(chars[i + 8]) { continue; }
        // Word boundary after.
        if i + 9 < chars.len() && chars[i + 9].is_ascii_alphanumeric() { continue; }
        return true;
    }
    false
}
...
fn matches_uk_sort_code(s: &str) -> bool {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        for j in 0..chars.len().saturating_sub(7) {
            if chars[j].is_ascii_digit() && chars[j + 1].is_ascii_digit() && chars[j + 2] == '-'
                && chars[j + 3].is_ascii_digit() && chars[j + 4].is_ascii_digit() && chars[j + 5] == '-'
                && chars[j + 6].is_ascii_digit() && chars[j + 7].is_ascii_digit()
            {
                // Word boundary before (ignore digit-following chars).
                if j > 0 && chars[j - 1].is_ascii_alphanumeric() { continue; }
                // Word boundary after.
                if j + 8 < chars.len() && chars[j + 8].is_ascii_alphanumeric() { continue; }
            ...
            if words.len() >= 2 {
                let bigram_match = words[..2].iter().all(|w| {
                    ...
                    chars[0].is_ascii_uppercase()
                        && chars.iter().skip(1).all(|c| c.is_ascii_lowercase() || *c == '-' || *c == '\'')
                        && chars.len() >= 2
                });
                if bigram_match { return true; }
            }
        }
    }
    false
...
fn scan_for_pii(content: &str) -> Vec<(&'static str, usize)> {
    ...
    for (i, line) in content.lines().enumerate() {
        if matches_email(line) { hits.push(("email", i + 1)); }
        if matches_uk_ni(line) { hits.push(("uk_ni", i + 1)); }
    }
    if matches_uk_sort_code(content) { hits.push(("uk_sort_code", 0)); }
    if matches_named_person(content) { hits.push(("named_person", 0)); }
    hits
}
...
fn has_valid_override(content: &str) -> bool {
    ...
    let reason_line = content.lines().find(|l| l.trim_start().starts_with("tier_override_reason:"));
    if let Some(line) = reason_line {
        let value = line.split("tier_override_reason:").nth(1).unwrap_or("").trim();
...
fn pii_scan_detects_email() {
    ...
    assert!(hits.iter().any(|(p, _)| *p == "email"), "email pattern should fire on `alice@example.com`; got hits: {hits:?}");
}
...
fn pii_scan_detects_uk_ni_number() {
    ...
    let content = fs::read_to_string(&file).unwrap();
    let hits = scan_for_pii(&content);
    assert!(hits.iter().any(|(p, _)| *p == "uk_ni"), "UK NI pattern should fire on `AB123456C`; got hits: {hits:?}");
}
...
fn pii_scan_detects_uk_sort_code_with_context() {
    ...
    let hits = scan_for_pii(content);
    assert!(hits.iter().any(|(p, _)| *p == "uk_sort_code"), "UK sort code with `sort code` context should fire; got hits: {hits:?}");
}
...
fn pii_scan_skips_uk_sort_code_without_context() {
    ...
    let hits = scan_for_pii(content);
    assert!(!hits.iter().any(|(p, _)| *p == "uk_sort_code"), "UK sort code without context literal should NOT fire; got hits: {hits:?}");
}
...
fn pii_scan_detects_named_person() {
    ...
    let hits = scan_for_pii(content);
    assert!(hits.iter().any(|(p, _)| *p == "named_person"), "named_person should fire on `name: Alice Smith`; got hits: {hits:?}");
}
...
fn pii_scan_does_not_fire_on_placeholders() {
    ...
    let hits = scan_for_pii(content);
    assert!(hits.is_empty(), "placeholders should not trigger PII matches; got hits: {hits:?}");
}
...
"#;
    ...
    assert!(has_valid_override(content), "override frontmatter must be recognised as valid");
}
...
fn pii_scan_override_without_reason_rejected() {
    ...
    assert!(!has_valid_override(content_no_reason), "override without tier_override_reason must be rejected");

    let content_placeholder_reason = "---\npii_scan_override: true\ntier_override_reason: <one-line rationale>\n---\n";
    assert!(!has_valid_override(content_placeholder_reason), "override with placeholder-template reason text must be rejected");
}
...
fn pii_scan_walks_tempdir_classifies_files() {
    ...
    assert_eq!(violators.len(), 1, "expected exactly 1 violator; got {violators:?}");
...
fn slo_verify_pass_4_documents_all_four_pattern_types() {
    ...
    let pattern_signals = ["Email addresses", "National Insurance", "sort code", "Capitalised-bigram"];
