...
fn slo_founder_check_documents_12_question_self_assessment() {
    ...
    assert!(
        any,
        "/slo-founder-check must document 12-question self-assessment"
    );
}
...
fn slo_founder_check_documents_worst_case_runway_tiers() {
    ...
    for t in &tiers {
        assert!(
            s.contains(t),
            "/slo-founder-check must document worst-case-runway `{t}`"
        );
    }
}
...
fn slo_founder_check_has_yc_application_prep() {
    ...
    let signals = [
        "YC application prep",
        "YC's standard application",
        "YC application",
    ];
    let any = signals.iter().any(|x| s.contains(x));
    assert!(
        any,
        "/slo-founder-check must offer optional YC application prep"
    );
}
...
fn pack_level_claude_md_catalogs_all_fifteen() {
    ...
    for name in ALL_FIFTEEN_BIZ_SKILLS {
        ...
        assert!(
            claude.contains(&verb),
            "CLAUDE.md must catalog all 15 biz skills; missing `{verb}`"
        );
    }
}
