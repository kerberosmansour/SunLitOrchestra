...
fn slo_hire_mandatory_ir35_triage_gate() {
    let s = read(&repo_root().join("skills/slo-hire/SKILL.md"));
    assert!(s.contains("ir35-cest-factors.md"), "/slo-hire must cite the IR35 reference");
    let signals = ["MANDATORY", "Mandatory", "seven IR35", "seven hard-block-to-lawyer IR35"];
...
fn slo_hire_documents_seven_ir35_triggers() {
    let s = read(&repo_root().join("skills/slo-hire/SKILL.md"));
    let signals = ["Substitution clause", "Full-time", "Exclusive engagement", "CEST"];
    let count = signals.iter().filter(|x| s.contains(**x)).count();
    assert!(count >= 3, "/slo-hire must enumerate IR35 triggers (found {count})");
}
...
fn slo_hire_rejects_tax_efficiency_framing() {
    ...
    let any = signals.iter().any(|x| s.contains(x));
    assert!(any, "/slo-hire must reject 'call them a contractor for tax efficiency' framing");
}
