...
fn get_api_docs_skill_md_exists_and_is_valid() {
    let body = fs::read_to_string(skill_dir().join("SKILL.md"))
        .expect("get-api-docs SKILL.md missing");
...
fn upstream_md_present_with_source_and_commit() {
    ...
    assert!(body.contains("andrewyng/context-hub"), "must cite upstream repo");
    assert!(
    ...
    assert!(
        body.contains("596506ebb4d53cfbc6ae458b731e0b1a18790f9e")
            || body.contains("Fetched on"),
...
fn get_api_docs_installs_through_generic_pickup() {
    ...
    let link = home.path().join(".claude").join("skills").join("get-api-docs");
    assert!(
        link.is_symlink(),
...
fn claude_md_lists_get_api_docs() {
    let claude_md = fs::read_to_string(repo_root().join("CLAUDE.md"))
