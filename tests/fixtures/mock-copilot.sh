#!/usr/bin/env bash
# mock-copilot.sh — Mimics the copilot CLI for integration testing.
#
...
# testing without requiring actual Copilot API access.
set -euo pipefail
...
# or "write the runbook output to `<path>`".
OUTPUT_PATH=""
...
| 2 | Document feature | — |
RUNBOOK
    echo "mock-copilot: wrote runbook to $OUTPUT_PATH"
else
    # For run-milestones prompts, just print success
    echo "mock-copilot: processed prompt (model=$MODEL)"
fi
