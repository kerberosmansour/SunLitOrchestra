...
fn parse_v3_template_tracker() {
    ...
    let template_path =
        Path::new(manifest_dir).join("docs/slo/templates/runbook-template_v_3_template.md");
...
fn parse_actual_runbook_tracker() {
    ...
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let runbook_path =
        Path::new(manifest_dir).join("docs/slo/completed/RUNBOOK-BIZ-SKILL-PACK-A.md");
